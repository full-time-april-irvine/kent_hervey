...
def wall():
    ...
    if 'user_id' not in session:
        ...
        numberRXmess=len(send_users) #send users is a list of dictionaries.  len obtains the number of items in the list
        print(send_users)
        print("["*77)
        print(numberRXmess)
...
############need the [0] and the key or else I am not really getting the user id

    print(messIDcheck)
    print("ee"*88)
    print(session['user_id'])
    print(messIDcheck[0]['receiver_id'])
    if session['user_id'] != messIDcheck[0]['receiver_id']:
