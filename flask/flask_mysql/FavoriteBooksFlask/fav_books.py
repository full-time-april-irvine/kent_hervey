...
#there are three html pages that look different only by flash messages and personalization such as using users name and populated data fields
    ...
    #allBooks.html is used for displaying All books whether favorited or not  This is a success page
    #oneBook.html   displays single book 
...
@app.route('/books')
def books(): #like a success route this route is the result of a redirect...so it renders a template, but also the user can use 5000/books as a bookmark as the entry place which will only work if they are still logged in
def books():
